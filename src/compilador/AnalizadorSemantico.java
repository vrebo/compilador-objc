...
public class AnalizadorSemantico {
    ...
    public void ejecutarAnalisisSemantico(PanelPrincipal panel) {
                    ...
                    if (esDefinicionFuncion(finArgumentos, tokens)) {
                        ...
                        for (int j = finArgumentos; j < finDefinicion; j++) {
                            if (tiposDatos.contains(tokens[j].trim())) {
                                agregarVariablesLocales(tokens, j, variablesLocales, argumentos, mensajes, mensajes);
                            }
                        }
                        ...
                        if (!funciones.containsKey(funcion.getIdentificador())) {
                            ...
                            if (!args.equals(args2)) {
                                System.out.println("Funcion " + funcion.getIdentificador()
                                        + " agregada con parámetros " + args);
                                mensajes.append("Funcion ").append(funcion.getIdentificador()).append(" agregada con parámetros ").append(args).append("\n");
                                funciones.put(funcion.getIdentificador(), funcion);
                                ...
                                System.out.println("Funcion definida (previamente declarada) "
                                        ...
                                        + " agregada con parámetros " + args);
                                mensajes.append("Funcion definida (previamente declarada) ").append(funcion.getIdentificador()).append(" agregada con parámetros ").append(args).append("\n");
                                funciones.put(funcion.getIdentificador(), funcion);
                            } else {
                                mensajes.append("Funcion ").append(funcion.getIdentificador()).append(" ya existente con argumentos ").append(args).append("\n");
                            }
                        }
                        i = finDefinicion;
                    } else {
                        ...
                        if (!flag) {
                            mensajes.append("Funcion ").append(funcion.getIdentificador()).append(" ya existente con parametros ").append(args).append("\n");
                        }
                        i = finArgumentos;
                    }
                } else {
                    ...
                    for (int j = i; j < fin; j++) {
                        ...
                        if (bandera) {
                            ...
                            if (!variables.containsKey(id.getId())) {
                                ...
                                mensajes.append("Variable ").append(id.getId()).append(" de tipo ").append(id.getTipo()).append(" agregada").append("\n");
                                variables.put(identificador, id);
                            } else {
                                System.out.println("Variable " + id.getId()
                                        + " ya contenida de tipo " + variables.get(id.getId()).getTipo());
                                mensajes.append("Variable ").append(id.getId()).append(" ya contenida de tipo ").append(variables.get(id.getId()).getTipo()).append("\n");
                            }
                            bandera = false;
                        }
                    }
        ...
        javax.swing.JTextArea salida = panel.getPanelError().getJtArea();
